...
import { ArrowUpRight, Blocks, BriefcaseBusiness, CircleUserRound, Home, Mail, Radio, X, Scan, ArrowLeft } from 'lucide-react';
...
const destinations = [{ id: 'projects', label: 'Projects', sub: 'Ideas into things', icon: BriefcaseBusiness }, { id: 'about', label: 'About', sub: 'Same human. More ideas.', icon: CircleUserRound }, { id: 'leads', label: 'Leads', sub: 'Let’s make something.', icon: Mail }, { id: 'socials', label: 'Socials', sub: 'Find me around', icon: Radio }] as const;
...
export default function Portfolio() {
 ...
 <header className="topbar"><button className="brand" onClick={() => visit('overview')} aria-label="Indie Block overview"><Blocks size={27}/><span>INDIE BLOCK<small>A LITTLE WORLD BY {profile.alias}</small></span></button><div className="top-right"><span className="status-dot"/>OPEN TO CONNECTIONS<button className="view-toggle" onClick={() => { if (fallback) { setFallback(false); setReady(false); } else { onFailure(); } }}>{fallback ? 'Explore in 3D' : 'Read without 3D'}</button><span className="edition">VOL. 01 / PORTFOLIO</span></div></header>
 {!ready && <div className="loading" role="status"><Blocks size={40}/><h2>INDIE BLOCK</h2><p>BUILDING THE NEIGHBORHOOD</p><div className="loading-track"><span /></div></div>}
 {section === 'overview' && <><div className="intro"><span className="eyebrow">DEVELOPER BY TRADE. BUILDER AT HEART.</span><h1>A small block.<br />A world of ideas<span>.</span></h1><p>{profile.name} <span>/</span> {profile.role}</p></div><div className="world-caption"><span className="coordinate">INDIE BLOCK / ROOFTOP DISTRICT</span><span>FOUR ROOFTOPS. ONE CURIOUS MIND.</span></div>{fallback && <div className="fallback-card"><h2>Welcome to Indie Block.</h2><p>The city is taking a breather. All projects, stories, and ways to connect are available below.</p>{destinations.map(d => <button key={d.id} onClick={() => visit(d.id)}>{d.label}<ArrowUpRight size={18}/></button>)}</div>}</>}
 {active && fallback && <section className="content-panel" aria-labelledby="section-heading"><div className="panel-top"><span className="eyebrow">INDIE BLOCK / {String(destinations.findIndex(d => d.id === section) + 1).padStart(2, '0')}</span><button className="icon-button" onClick={() => visit('overview')} aria-label="Back to Indie Block"><X size={20}/></button></div><h2 ref={heading} tabIndex={-1} id="section-heading">{active.label}<span>↗</span></h2><div className="panel-subtitle">{active.sub}</div>{section === 'projects' && <Projects />}{section === 'about' && <About />}{section === 'leads' && <ContactForm />}{section === 'socials' && <><h3>Same sky. Different stories.</h3><p>Around the internet, usually building, exploring, or finding the next little thing.</p><div className="social-list">{socials.map(s => <a href={s.url} key={s.name} target="_blank" rel="noopener noreferrer"><span className="social-mark"><SocialAppLogo name={s.name} size={36}/></span><span><strong>{s.name}</strong><small>{s.label}</small></span><ArrowUpRight size={22}/></a>)}</div></>}<button className="back-link" onClick={() => visit('overview')}><ArrowLeft size={15}/> Back to Indie Block</button></section>}
 {active && !fallback && <div className="rooftop-navigation"><button onClick={() => visit('overview')}><ArrowLeft size={15}/> Back to Indie Block</button><span aria-live="polite">{transitioning ? `Travelling to ${active.label}…` : `${active.label} / drag to rotate · use the rooftop displays`}</span></div>}
